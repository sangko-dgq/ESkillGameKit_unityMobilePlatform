...
public class SkillArea : MonoBehaviour
{
    ...
    [Header("左手轮盘")] public VariableJoystick variableJoystick;


    void Update()
    {
        //技能转盘按到到才执行朝向更新，防止和左手方向盘更新冲突。 isPressed == true
        //如果左手没有在转动轮盘             direction2 == new Vector3(0, 0, 0)
        Vector3 direction2 = Vector3.forward * variableJoystick.Vertical + Vector3.right * variableJoystick.Horizontal;
        if (isPressed == true && direction2 == new Vector3(0, 0, 0))
            GameObject.FindGameObjectWithTag("OBJ_Player").transform.LookAt(this.GetCirclePosition(100f)); //以右手技能方向轮盘实现朝向为准
    }
